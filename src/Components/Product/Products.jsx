...
const Products = ({ product, addToCart, setAddToCart }) => {
  ...
  return (
    <>
      <div className="product space-y-4 p-6 shadow rounded-2xl flex flex-col relative transition-all duration-300 hover:-translate-y-2 hover:shadow-xl">
        <div className="flex-1">
          <img src={product.icon} alt="" />
        </div>
        <div>
          <span
            className={`rounded-full px-3 py-1 ${product.tag == "Popular" ? " bg-purple-100 text-purple-500" : product.tag == "Best Seller" ? "bg-amber-100 text-red-500" : "bg-green-100 text-green-500"} absolute top-2 right-2`}
          >
            {product.tag}
          </span>
        </div>
